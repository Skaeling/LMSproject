...
<h2>Настройки удаленного сервера и деплоя:</h2>
<ol>
  ...
  <li>Для активации автоматического деплоя с загрузкой образа добавьте в секреты DEPLOY_DIR, и задайте в нем абсолютный путь к корневой директории проекта (путь до файла /.env не включая слеш)</li>
  <li>Для запуска workflow также потребуются следующие секреты: secrets.SSH_key, DJANGO_SECRET_KEY, DOCKER_HUB_ACCESS_TOKEN, DOCKER_HUB_USERNAME, SSH_USER, SERVER_IP</li>
